...
struct SettingsOverlay: View {
    ...
    var body: some View {
        ZStack {
            // Scrim: consistent with EditorOverlay
            Color.black.opacity(max(0.7, viewModel.bgOpacity))
                .ignoresSafeArea()
            ...
            VStack(spacing: 0) {
                ...
                HStack {
                    ZStack {
                        Circle().fill(viewModel.accentColor.opacity(0.15)).frame(width: 32, height: 32)
                    ...
                    // Tab Switcher - Optimized for AI focus
                    HStack(spacing: 4) {
                        tabButton(title: "模型厂商", icon: "server.rack", index: 0)
                        tabButton(title: "角色库", icon: "person.text.rectangle", index: 1)
                        tabButton(title: "上下文", icon: "brain.head.profile", index: 2)
                        tabButton(title: "热键配置", icon: "keyboard", index: 3)
                        tabButton(title: "外观呈现", icon: "paintpalette", index: 4)
                        tabButton(title: "隐私安全", icon: "shield.fill", index: 5)
                        tabButton(title: "高级设置", icon: "gearshape.2", index: 6)
                    }
                    .padding(4)
                ...
                HStack {
                    if selectedTab == 3 {
                        ...
                        HStack(spacing: 6) {
                            Circle()
                                .fill(viewModel.activeStatusColor)
                ...
                .animation(.spring(), value: viewModel.showSaveStatus)
                .padding(24)
            }
            ...
            .background {
                ZStack {
                    Color.black.opacity(viewModel.bgOpacity)
...
private struct AIProvidersTab: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 16) {
                // Failover Settings
                SettingsSection(title: "故障转移设置", icon: "arrow.triangle.2.circlepath") {
                                        ...
                                        HStack(spacing: 8) {
                                            Button(action: {
                                                ...
                                                Image(systemName: "chevron.up")
                                                    .font(.system(size: 10))
                                                    .foregroundColor(index > 0 ? .white.opacity(0.4) : .white.opacity(0.15))
                                            }
                                            .buttonStyle(.plain)
                                            .disabled(index == 0)
                                            ...
                                            Button(action: {
                                                ...
                                                Image(systemName: "chevron.down")
                                                    .font(.system(size: 10))
                                                    .foregroundColor(index < viewModel.providerPriority.count - 1 ? .white.opacity(0.4) : .white.opacity(0.15))
                                            }
                                            .buttonStyle(.plain)
...
private struct ProviderCard: View {
    ...
    var body: some View {
                ...
                HStack {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        HStack(spacing: 8) {
                            ...
                            if isSelected {
                                Text("当前默认")
                                    ...
                                    .padding(.vertical, 2)
                                    .background(viewModel.accentColor.opacity(0.3))
                                    .foregroundColor(viewModel.accentColor)
                                    .clipShape(Capsule())
                            }
                        }
                        ...
                        Text(viewModel.getProviderStatus(provider))
                            .font(.system(size: 10))
                            .foregroundColor(viewModel.getProviderStatusColor(provider))
                    }
                    ...
                    if !isSelected {
                        ...
                        .padding(.vertical, 6)
                        .background(viewModel.accentColor.opacity(0.1), in: RoundedRectangle(cornerRadius: 6))
                    }
                }
                .padding(16)
                .contentShape(Rectangle())
                ...
                VStack(spacing: 14) {
                    ...
                    HStack(alignment: .center, spacing: 12) {
                        Button(action: {
                            ...
                            .padding(.vertical, 6)
                            .background(viewModel.accentColor.opacity(0.2), in: RoundedRectangle(cornerRadius: 6))
                            .foregroundColor(viewModel.accentColor)
                        }
                        .buttonStyle(.plain)
                        ...
                        if let status = viewModel.apiTestStatus, isSelected {
                            Text(status)
                                .font(.system(size: 10))
                                .foregroundColor(status.contains("成功") ? .green : .orange)
                        }
                    }
                }
                .padding(16)
        ...
        .overlay(RoundedRectangle(cornerRadius: 12).stroke(isSelected ? viewModel.accentColor.opacity(0.5) : Color.white.opacity(0.1), lineWidth: 1))
        .onAppear {
                        ...
                        ForEach(0..<viewModel.aiRoles.count, id: \.self) { index in
                            Button(action: { viewModel.selectedRoleIndex = index }) {
                                Text(viewModel.aiRoles[index].name)
                                    .font(.system(size: 11, weight: viewModel.selectedRoleIndex == index ? .bold : .medium))
                                    .padding(.horizontal, 12)
                                    .padding(.vertical, 8)
                                    .background(viewModel.selectedRoleIndex == index ? viewModel.accentColor.opacity(0.3) : Color.white.opacity(0.05))
...
private struct AIContextTab: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                SettingsSection(title: "背景资料控制", icon: "doc.text") {
                    VStack(spacing: 12) {
                        HStack {
                            VStack(alignment: .leading, spacing: 4) {
                                Text("使用背景资料")
                ...
                SettingsSection(title: "全局个人风格记忆", icon: "text.bubble") {
                    VStack(alignment: .leading, spacing: 8) {
...
private struct AppearanceSettingsTab: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 18) {
                SettingsSection(title: "视觉呈现", icon: "textformat") {
                    VStack(spacing: 14) {
                        ...
                        HStack {
                            VStack(alignment: .leading, spacing: 2) {
                                Text("代码模式 (Code Mode)").foregroundColor(.white.opacity(0.8))
...
private struct PrivacySettingsTab: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 18) {
                SettingsSection(title: "隐私与隐身", icon: "shield.fill") {
                    VStack(spacing: 16) {
                        HStack(alignment: .center) {
                            VStack(alignment: .leading, spacing: 2) {
                                Text("隐私防护 (Privacy Mode)").foregroundColor(.white.opacity(0.8))
                                Text("隐藏 Dock 图标并启用录屏抓取防护")
                                    .font(.system(size: 10)).foregroundColor(.white.opacity(0.4))
                            }
                            Spacer()
                            Toggle("", isOn: $viewModel.isPrivacyMode)
                                .toggleStyle(SwitchToggleStyle(tint: viewModel.accentColor))
                                .labelsHidden()
                        }
                        ...
                        HStack(alignment: .center, spacing: 14) {
                            VStack(alignment: .leading, spacing: 3) {
                                ...
                                Text(viewModel.mousePenetration ? "幽灵模式已锁定 - 倒计时结束释放" : "鼠标直接穿过提词器，操作背后窗口")
                                    .font(.system(size: 10))
                                    .foregroundColor(viewModel.mousePenetration ? .orange : .white.opacity(0.42))
                            }
                            Spacer()
                            ...
                            if viewModel.isGhostModePending {
                                ...
                                Button(action: { viewModel.requestGhostMode() }) {
                                    Text("启动锁定会话")
                                        ...
                                        .padding(.horizontal, 12)
                                        .padding(.vertical, 6)
                                        .background(viewModel.accentColor, in: RoundedRectangle(cornerRadius: 6))
                                }
                                .buttonStyle(.plain)
                            }
                        }
                ...
                HStack(spacing: 6) {
                    ForEach(presetColors(for: selection.wrappedValue), id: \.self) { color in
                        Circle()
                            ...
                            .overlay(
                                Circle()
                                    .stroke(compareColors(selection.wrappedValue, color) ? Color.white.opacity(0.9) : Color.clear, lineWidth: 2)
                            )
...
private struct AdvancedSettingsTab: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 24) {
                SettingsSection(title: "AI 智播高级设置", icon: "sparkles.square.filled.on.square") {
                    VStack(spacing: 16) {
                        HStack {
                            VStack(alignment: .leading, spacing: 4) {
                                Text("自动追随滚动")
                ...
                SettingsSection(title: "数据与初始化", icon: "internaldrive") {
                    VStack(spacing: 0) {
    ...
    private func settingActionRow(title: String, icon: String, isDestructive: Bool = false, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            HStack(spacing: 12) {
                Image(systemName: icon)
                    .foregroundColor(isDestructive ? .red : .white.opacity(0.5))
                    .frame(width: 20)
                Text(title)
                    .font(.system(size: 13))
            ...
            if isRecording {
                ...
                HStack(spacing: 4) {
                    ...
                    Text(shortcut.key.uppercased() == " " ? "SPACE" : shortcut.key.uppercased())
                        ...
                        .padding(.horizontal, 6)
                        .padding(.vertical, 3)
                        .background(viewModel.accentColor.opacity(0.2), in: RoundedRectangle(cornerRadius: 4))
                        .overlay(RoundedRectangle(cornerRadius: 4).stroke(viewModel.accentColor.opacity(0.3), lineWidth: 1))
                        .foregroundColor(.white)
                }
            } else {
                Text("未设置")
